...
We consume Avro through `pkg/wire` (our own OCF codec, which imports both
`github.com/hamba/avro/v2` and `github.com/hamba/avro/v2/ocf`). On branches that
also build `github.com/apache/iceberg-go`, iceberg-go imports
`github.com/hamba/avro/v2` directly for manifest read/write; this branch
(`feat/managed-snowflake-wip`) does not yet build iceberg-go, so `pkg/wire` is
the sole consumer. The remediation is identical either way.
...
`github.com/iskorotkov/avro/v2/...` internally. The replaced-in source still
imports the `iskorotkov` root package, so the same module version supplies two
module paths at once. Even with `pkg/wire` as the only direct consumer, that
means a consumer would hold `hamba/avro/v2.Schema` while the fork's `ocf`
package produced `iskorotkov/avro/v2.Schema` — **two incompatible Avro type
universes** that do not compile, and, if forced, would silently split the type
system.
