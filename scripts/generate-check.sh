...
if ! git diff --exit-code HEAD -- gen/go; then
	echo "generated protobuf stubs are stale; run make generate and include the results" >&2
	exit 1
fi
...
if [ "${CI:-}" = "true" ] && [ -n "$untracked" ]; then
	echo "generated protobuf stubs are untracked in CI; run make generate and commit them" >&2
