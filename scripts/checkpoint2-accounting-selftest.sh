...
while IFS='|' read -r profile test_name; do
  ...
  case "${profile}" in
    ...
    *) echo "promoted profile ${profile} has no strict justfile gate" >&2; exit 1 ;;
  esac
...
if [[ -n "${results}" || -n "${selected_profile}" ]]; then
  [[ -n "${results}" && -n "${selected_profile}" ]] || { echo '--results and --profile must be supplied together' >&2; exit 2; }
  required=$(printf '%s\n' "${profile_evidence}" | awk -F'|' -v profile="${selected_profile}" '$1==profile { values=(values ? values "," : "") $2 } END { print values }')
  [[ -n "${required}" ]] || { echo "no promoted evidence for ${selected_profile}" >&2; exit 1; }
