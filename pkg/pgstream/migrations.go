...
const migrationsTableSQL = `CREATE TABLE IF NOT EXISTS wallaby_stream_migrations (
...
func runMigrations(ctx context.Context, pool *pgxpool.Pool) error {
	...
	for _, file := range files {
		...
		if _, err := pool.Exec(ctx, "INSERT INTO wallaby_stream_migrations (version) VALUES ($1)", version); err != nil {
...
func loadAppliedMigrations(ctx context.Context, pool *pgxpool.Pool) (map[string]bool, error) {
	rows, err := pool.Query(ctx, "SELECT version FROM wallaby_stream_migrations")
	if err != nil {
