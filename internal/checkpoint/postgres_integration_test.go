...
func TestPostgresStoreRejectsCheckpointRegression(t *testing.T) {
	...
	entry := connector.OutboxEntry{Destination: "secondary", PositionID: position, Batch: batch}
	...
	err = store.PersistCheckpointAndOutbox(ctx, outboxFlow, checkpoint, []connector.OutboxEntry{{Destination: "secondary", PositionID: position, Batch: conflicting}})
