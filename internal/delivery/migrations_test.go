...
func TestManagedPartReservationMigrationRequiresAuthorityV2(t *testing.T) {
	raw, err := migrationFS.ReadFile("migrations/011_managed_part_reservations.sql")
	if err != nil {
		t.Fatal(err)
	}
	source := string(raw)
	for _, required := range []string{"CREATE TABLE managed_part_reservations", "CREATE TABLE managed_part_reservation_parts", "managed_part_reservations_manifest_fkey", "managed_part_reservations_budget_idx", "wallaby_require_authority_protocol_v2", "reservation_state IN ('reserved','completed_pending_observation','reclaim_pending','released')", "part_state IN ('reserved','durable','released')", "charge_state IN ('charged','observed','released')", "CREATE TABLE managed_part_reservation_events"} {
		if !strings.Contains(source, required) {
			t.Fatalf("managed part reservation migration missing %q", required)
		}
	}
	for _, forbidden := range []string{"IF NOT EXISTS", "legacy", "fallback"} {
		if strings.Contains(source, forbidden) {
			t.Fatalf("managed part reservation migration contains compatibility token %q", forbidden)
		}
	}
}
