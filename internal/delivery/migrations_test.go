...
func TestSchemaBaselineManifestMigrationRefusesInference(t *testing.T) {
	raw, err := migrationFS.ReadFile("migrations/009_schema_baseline_manifest.sql")
	if err != nil {
		t.Fatal(err)
	}
	source := string(raw)
