...
func TestSharedControlMigrationsPrecedeComponentConstruction(t *testing.T) {
	...
	for _, constructor := range []string{"authority.NewPostgresStore(controlPool)", "delivery.NewCoordinator(ctx, controlPool)", "workflow.NewPostgresEngineWithPool(ctx, controlPool)", "registry.NewPostgresStoreWithPool(ctx, controlPool)", "pgstream.NewStore(ctx, cfg.Postgres.DSN)"} {
