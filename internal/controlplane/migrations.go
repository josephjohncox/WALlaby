...
var exactAuthorityConstraints = []exactAuthorityConstraint{
	...
	{table: "managed_part_reservations", name: "managed_part_reservations_manifest_fkey", kind: "f", noInherit: true, columns: []string{"flow_incarnation_id", "destination_revision_id", "logical_batch_id"}, definition: "FOREIGN KEY (flow_incarnation_id, destination_revision_id, logical_batch_id) REFERENCES delivery_manifests(flow_incarnation_id, destination_revision_id, logical_batch_id) ON DELETE RESTRICT"},
...
func verifyExactAuthoritySchema(ctx context.Context, pool authorityCatalogQueryer) error {
	...
	for _, table := range []string{"delivery_manifests", "managed_part_reservations", "managed_part_reservation_parts", "managed_part_reservation_events", "artifact_delivery_attempts", "artifact_delivery_receipts", "artifact_consumer_checkpoints"} {
