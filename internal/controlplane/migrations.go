...
var authorityMutableTables = []string{
	...
	"destination_revisions", "delivery_manifests", "delivery_attempts", "delivery_attempt_evidence", "delivery_receipts", "source_ack_intents", "source_ack_receipts", "delivery_retention_roots", "source_ack_retention_roots", "managed_part_reservations", "managed_part_reservation_parts", "managed_part_reservation_events",
...
var requiredManagedColumns = map[string][]string{
	...
	"managed_part_reservations":       {"reservation_id", "flow_incarnation_id", "flow_id", "generation", "acquisition_id", "lease_epoch", "destination_revision_id", "source_lineage_id", "logical_batch_id", "position_id", "content_hash", "plan_hash", "resource", "server_active_parts", "planned_parts", "capacity", "reservation_epoch", "reclaim_epoch", "observation_epoch", "reservation_state", "observed_at", "completed_at", "reclaim_started_at", "released_at"},
	"managed_part_reservation_parts":  {"reservation_id", "part_kind", "part_ordinal", "query_id", "part_state", "charge_state", "durable_at", "observed_at", "released_at"},
	"managed_part_reservation_events": {"event_id", "reservation_id", "reservation_epoch", "observation_epoch", "reclaim_epoch", "event_kind", "generation", "acquisition_id", "lease_epoch", "server_active_parts", "charged_parts", "created_at"},
...
var exactAuthorityColumns = map[string][]exactManagedColumn{
	...
	"managed_part_reservations": {
		{name: "reservation_id", dataType: "uuid", notNull: true},
		{name: "flow_incarnation_id", dataType: "uuid", notNull: true},
		{name: "flow_id", dataType: "text", notNull: true},
		...
		{name: "position_id", dataType: "text", notNull: true},
		{name: "content_hash", dataType: "text", notNull: true},
		{name: "plan_hash", dataType: "text", notNull: true},
		{name: "resource", dataType: "text", notNull: true},
		{name: "server_active_parts", dataType: "bigint", notNull: true},
		{name: "planned_parts", dataType: "integer", notNull: true},
		{name: "capacity", dataType: "bigint", notNull: true},
		{name: "reservation_epoch", dataType: "bigint", notNull: true, defaultExpr: "1"},
		{name: "reclaim_epoch", dataType: "bigint", notNull: true, defaultExpr: "0"},
		{name: "observation_epoch", dataType: "bigint", notNull: true, defaultExpr: "1"},
		{name: "reservation_state", dataType: "text", notNull: true, defaultExpr: "'reserved'::text"},
		{name: "created_at", dataType: "timestamp with time zone", notNull: true, defaultExpr: "clock_timestamp()"},
		{name: "updated_at", dataType: "timestamp with time zone", notNull: true, defaultExpr: "clock_timestamp()"},
		{name: "observed_at", dataType: "timestamp with time zone", notNull: true, defaultExpr: "clock_timestamp()"},
		{name: "completed_at", dataType: "timestamp with time zone"},
		{name: "reclaim_started_at", dataType: "timestamp with time zone"},
		{name: "released_at", dataType: "timestamp with time zone"},
	},
	"managed_part_reservation_parts": {
		{name: "reservation_id", dataType: "uuid", notNull: true},
		{name: "part_kind", dataType: "text", notNull: true},
		{name: "part_ordinal", dataType: "bigint", notNull: true},
		{name: "query_id", dataType: "text", notNull: true},
		{name: "part_state", dataType: "text", notNull: true, defaultExpr: "'reserved'::text"},
		{name: "charge_state", dataType: "text", notNull: true, defaultExpr: "'charged'::text"},
		{name: "durable_at", dataType: "timestamp with time zone"},
		{name: "observed_at", dataType: "timestamp with time zone"},
		{name: "released_at", dataType: "timestamp with time zone"},
		{name: "updated_at", dataType: "timestamp with time zone", notNull: true, defaultExpr: "clock_timestamp()"},
	},
	"managed_part_reservation_events": {
		{name: "event_id", dataType: "bigint", notNull: true, identity: "a"},
		{name: "reservation_id", dataType: "uuid", notNull: true},
		{name: "reservation_epoch", dataType: "bigint", notNull: true},
		{name: "observation_epoch", dataType: "bigint", notNull: true},
		{name: "reclaim_epoch", dataType: "bigint", notNull: true},
		{name: "event_kind", dataType: "text", notNull: true},
		{name: "generation", dataType: "bigint", notNull: true},
		{name: "acquisition_id", dataType: "uuid", notNull: true},
		{name: "lease_epoch", dataType: "bigint", notNull: true},
		{name: "server_active_parts", dataType: "bigint", notNull: true},
		{name: "charged_parts", dataType: "bigint", notNull: true},
		{name: "created_at", dataType: "timestamp with time zone", notNull: true, defaultExpr: "clock_timestamp()"},
	},
...
var exactAuthorityConstraints = []exactAuthorityConstraint{
	...
	{table: "managed_part_reservations", name: "managed_part_reservations_capacity_check", kind: "c", columns: []string{"capacity"}, definition: "CHECK (capacity > 0)"},
	{table: "managed_part_reservations", name: "managed_part_reservations_destination_revision_id_fkey", kind: "f", noInherit: true, columns: []string{"destination_revision_id"}, definition: "FOREIGN KEY (destination_revision_id) REFERENCES destination_revisions(destination_revision_id) ON DELETE RESTRICT"},
	{table: "managed_part_reservations", name: "managed_part_reservations_generation_check", kind: "c", columns: []string{"generation"}, definition: "CHECK (generation > 0)"},
	{table: "managed_part_reservations", name: "managed_part_reservations_identity_key", kind: "u", noInherit: true, columns: []string{"destination_revision_id", "logical_batch_id"}, definition: "UNIQUE (destination_revision_id, logical_batch_id)"},
	{table: "managed_part_reservations", name: "managed_part_reservations_lease_epoch_check", kind: "c", columns: []string{"lease_epoch"}, definition: "CHECK (lease_epoch > 0)"},
	{table: "managed_part_reservations", name: "managed_part_reservations_manifest_fkey", kind: "f", noInherit: true, columns: []string{"flow_incarnation_id", "destination_revision_id", "position_id"}, definition: "FOREIGN KEY (flow_incarnation_id, destination_revision_id, position_id) REFERENCES delivery_manifests(flow_incarnation_id, destination_revision_id, position_id) ON DELETE RESTRICT"},
	{table: "managed_part_reservations", name: "managed_part_reservations_observation_epoch_check", kind: "c", columns: []string{"observation_epoch"}, definition: "CHECK (observation_epoch > 0)"},
	{table: "managed_part_reservations", name: "managed_part_reservations_pkey", kind: "p", noInherit: true, columns: []string{"reservation_id"}, definition: "PRIMARY KEY (reservation_id)"},
	{table: "managed_part_reservations", name: "managed_part_reservations_plan_hash_check", kind: "c", columns: []string{"plan_hash"}, definition: "CHECK (plan_hash ~ '^[0-9a-f]{64}$'::text)"},
	{table: "managed_part_reservations", name: "managed_part_reservations_planned_parts_check", kind: "c", columns: []string{"planned_parts"}, definition: "CHECK (planned_parts > 0)"},
	{table: "managed_part_reservations", name: "managed_part_reservations_reclaim_epoch_check", kind: "c", columns: []string{"reclaim_epoch"}, definition: "CHECK (reclaim_epoch >= 0)"},
	{table: "managed_part_reservations", name: "managed_part_reservations_reservation_epoch_check", kind: "c", columns: []string{"reservation_epoch"}, definition: "CHECK (reservation_epoch > 0)"},
	{table: "managed_part_reservations", name: "managed_part_reservations_reservation_state_check", kind: "c", columns: []string{"reservation_state"}, definition: "CHECK (reservation_state = ANY (ARRAY['reserved'::text, 'completed_pending_observation'::text, 'reclaim_pending'::text, 'released'::text]))"},
	{table: "managed_part_reservations", name: "managed_part_reservations_resource_check", kind: "c", columns: []string{"resource"}, definition: "CHECK (resource = 'clickhouse_active_parts_v1'::text)"},
	{table: "managed_part_reservations", name: "managed_part_reservations_server_active_parts_check", kind: "c", columns: []string{"server_active_parts"}, definition: "CHECK (server_active_parts >= 0)"},
	{table: "managed_part_reservations", name: "managed_part_reservations_state_complete", kind: "c", columns: []string{"reservation_state", "completed_at", "reclaim_started_at", "released_at"}, definition: "CHECK (reservation_state = 'reserved'::text AND completed_at IS NULL AND reclaim_started_at IS NULL AND released_at IS NULL OR reservation_state = 'completed_pending_observation'::text AND completed_at IS NOT NULL AND reclaim_started_at IS NULL AND released_at IS NULL OR reservation_state = 'reclaim_pending'::text AND completed_at IS NULL AND reclaim_started_at IS NOT NULL AND released_at IS NULL OR reservation_state = 'released'::text AND released_at IS NOT NULL)"},
	{table: "managed_part_reservation_parts", name: "managed_part_reservation_parts_charge_state_check", kind: "c", columns: []string{"charge_state"}, definition: "CHECK (charge_state = ANY (ARRAY['charged'::text, 'observed'::text, 'released'::text]))"},
	{table: "managed_part_reservation_parts", name: "managed_part_reservation_parts_observation_complete", kind: "c", columns: []string{"charge_state", "observed_at", "released_at"}, definition: "CHECK (charge_state = 'charged'::text AND observed_at IS NULL OR charge_state = 'observed'::text AND observed_at IS NOT NULL AND released_at IS NULL OR charge_state = 'released'::text AND released_at IS NOT NULL)"},
	{table: "managed_part_reservation_parts", name: "managed_part_reservation_parts_part_kind_check", kind: "c", columns: []string{"part_kind"}, definition: "CHECK (part_kind = ANY (ARRAY['changelog'::text, 'receipt'::text]))"},
	{table: "managed_part_reservation_parts", name: "managed_part_reservation_parts_part_ordinal_check", kind: "c", columns: []string{"part_ordinal"}, definition: "CHECK (part_ordinal >= 0)"},
	{table: "managed_part_reservation_parts", name: "managed_part_reservation_parts_part_state_check", kind: "c", columns: []string{"part_state"}, definition: "CHECK (part_state = ANY (ARRAY['reserved'::text, 'durable'::text, 'released'::text]))"},
	{table: "managed_part_reservation_parts", name: "managed_part_reservation_parts_pkey", kind: "p", noInherit: true, columns: []string{"reservation_id", "part_kind", "part_ordinal"}, definition: "PRIMARY KEY (reservation_id, part_kind, part_ordinal)"},
	{table: "managed_part_reservation_parts", name: "managed_part_reservation_parts_query_id_key", kind: "u", noInherit: true, columns: []string{"query_id"}, definition: "UNIQUE (query_id)"},
	{table: "managed_part_reservation_parts", name: "managed_part_reservation_parts_reservation_id_fkey", kind: "f", noInherit: true, columns: []string{"reservation_id"}, definition: "FOREIGN KEY (reservation_id) REFERENCES managed_part_reservations(reservation_id) ON DELETE RESTRICT"},
	{table: "managed_part_reservation_parts", name: "managed_part_reservation_parts_state_complete", kind: "c", columns: []string{"part_state", "durable_at", "charge_state", "observed_at", "released_at"}, definition: "CHECK (part_state = 'reserved'::text AND durable_at IS NULL AND charge_state = 'charged'::text AND observed_at IS NULL AND released_at IS NULL OR part_state = 'durable'::text AND durable_at IS NOT NULL AND (charge_state = ANY (ARRAY['charged'::text, 'observed'::text])) AND released_at IS NULL OR part_state = 'released'::text AND released_at IS NOT NULL AND charge_state = 'released'::text)"},
	{table: "managed_part_reservation_events", name: "managed_part_reservation_events_charged_parts_check", kind: "c", columns: []string{"charged_parts"}, definition: "CHECK (charged_parts >= 0)"},
	{table: "managed_part_reservation_events", name: "managed_part_reservation_events_event_kind_check", kind: "c", columns: []string{"event_kind"}, definition: "CHECK (event_kind = ANY (ARRAY['reserved'::text, 'adopted'::text, 'observed'::text, 'completed'::text, 'reclaim_started'::text, 'released'::text, 'rereserved'::text]))"},
	{table: "managed_part_reservation_events", name: "managed_part_reservation_events_generation_check", kind: "c", columns: []string{"generation"}, definition: "CHECK (generation > 0)"},
	{table: "managed_part_reservation_events", name: "managed_part_reservation_events_lease_epoch_check", kind: "c", columns: []string{"lease_epoch"}, definition: "CHECK (lease_epoch > 0)"},
	{table: "managed_part_reservation_events", name: "managed_part_reservation_events_observation_epoch_check", kind: "c", columns: []string{"observation_epoch"}, definition: "CHECK (observation_epoch > 0)"},
	{table: "managed_part_reservation_events", name: "managed_part_reservation_events_pkey", kind: "p", noInherit: true, columns: []string{"event_id"}, definition: "PRIMARY KEY (event_id)"},
	{table: "managed_part_reservation_events", name: "managed_part_reservation_events_reclaim_epoch_check", kind: "c", columns: []string{"reclaim_epoch"}, definition: "CHECK (reclaim_epoch >= 0)"},
	{table: "managed_part_reservation_events", name: "managed_part_reservation_events_reservation_epoch_check", kind: "c", columns: []string{"reservation_epoch"}, definition: "CHECK (reservation_epoch > 0)"},
	{table: "managed_part_reservation_events", name: "managed_part_reservation_events_reservation_id_fkey", kind: "f", noInherit: true, columns: []string{"reservation_id"}, definition: "FOREIGN KEY (reservation_id) REFERENCES managed_part_reservations(reservation_id) ON DELETE RESTRICT"},
	{table: "managed_part_reservation_events", name: "managed_part_reservation_events_server_active_parts_check", kind: "c", columns: []string{"server_active_parts"}, definition: "CHECK (server_active_parts >= 0)"},
...
var exactAuthorityIndexes = []exactAuthorityIndex{
	...
	{table: "managed_part_reservations", name: "managed_part_reservations_budget_idx", columns: []string{"destination_revision_id", "resource", "reservation_state"}, options: []int16{0, 0, 0}, predicate: "(reservation_state = ANY (ARRAY['reserved'::text, 'completed_pending_observation'::text, 'reclaim_pending'::text]))"},
	{table: "managed_part_reservations", name: "managed_part_reservations_identity_key", unique: true, columns: []string{"destination_revision_id", "logical_batch_id"}, options: []int16{0, 0}},
	{table: "managed_part_reservations", name: "managed_part_reservations_pkey", unique: true, primary: true, columns: []string{"reservation_id"}, options: []int16{0}},
	{table: "managed_part_reservation_parts", name: "managed_part_reservation_parts_pkey", unique: true, primary: true, columns: []string{"reservation_id", "part_kind", "part_ordinal"}, options: []int16{0, 0, 0}},
	{table: "managed_part_reservation_parts", name: "managed_part_reservation_parts_query_id_key", unique: true, columns: []string{"query_id"}, options: []int16{0}},
	{table: "managed_part_reservation_events", name: "managed_part_reservation_events_pkey", unique: true, primary: true, columns: []string{"event_id"}, options: []int16{0}},
	{table: "managed_part_reservation_events", name: "managed_part_reservation_events_reservation_idx", columns: []string{"reservation_id", "event_id"}, options: []int16{0, 0}},
...
var requiredManagedConstraints = []requiredManagedObject{
	...
	{table: "managed_part_reservations", name: "managed_part_reservations_pkey"},
	{table: "managed_part_reservations", name: "managed_part_reservations_destination_revision_id_fkey"},
	{table: "managed_part_reservations", name: "managed_part_reservations_manifest_fkey"},
	{table: "managed_part_reservations", name: "managed_part_reservations_identity_key"},
	{table: "managed_part_reservations", name: "managed_part_reservations_state_complete"},
	{table: "managed_part_reservation_parts", name: "managed_part_reservation_parts_pkey"},
	{table: "managed_part_reservation_parts", name: "managed_part_reservation_parts_query_id_key"},
	{table: "managed_part_reservation_parts", name: "managed_part_reservation_parts_reservation_id_fkey"},
	{table: "managed_part_reservation_parts", name: "managed_part_reservation_parts_state_complete"},
	{table: "managed_part_reservation_parts", name: "managed_part_reservation_parts_observation_complete"},
	{table: "managed_part_reservation_events", name: "managed_part_reservation_events_pkey"},
	{table: "managed_part_reservation_events", name: "managed_part_reservation_events_reservation_id_fkey"},
...
var requiredManagedIndexes = []requiredManagedObject{
	...
	{table: "managed_part_reservations", name: "managed_part_reservations_budget_idx"},
	{table: "managed_part_reservation_events", name: "managed_part_reservation_events_reservation_idx"},
...
func verifyExactAuthoritySchema(ctx context.Context, pool authorityCatalogQueryer) error {
	...
	for _, table := range []string{"delivery_manifests", "managed_part_reservations", "managed_part_reservation_parts", "managed_part_reservation_events", "artifact_delivery_attempts", "artifact_delivery_receipts", "artifact_consumer_checkpoints", "artifact_metadata_prune_claims"} {
