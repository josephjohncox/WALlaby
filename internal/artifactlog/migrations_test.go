...
func TestMetadataRetentionMigrationFreezesAndProtectsCatalogEvidence(t *testing.T) {
	raw, err := migrationFS.ReadFile("migrations/009_metadata_retention.sql")
	if err != nil {
		t.Fatal(err)
	}
	source := string(raw)
	for _, required := range []string{"catalog_evidence JSONB NOT NULL", "artifact_metadata_prune_claims_catalog_evidence_object", "wallaby_reject_metadata_prune_dependent", "artifact_deliveries_reject_metadata_prune", "artifact_delivery_attempts_reject_metadata_prune", "artifact_delivery_receipts_reject_metadata_prune", "artifact_gc_claims_publication_idx"} {
		if !strings.Contains(source, required) {
			t.Fatalf("metadata retention migration missing %q", required)
		}
	}
}
