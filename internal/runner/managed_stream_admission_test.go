...
func managedStreamingAdmissionFlowConfigured() (flow.Flow, StreamRunnerConfig) {
	...
	f.Source.Options["managed_profile"] = connector.ManagedProfilePostgresToSnowflakeStreamingRestAppendV1
...
func TestManagedAdmissionStreamingAppendFailsClosedWithoutTransport(t *testing.T) {
	...
	_, err := NewStreamRunner(f, &pgsource.Source{}, managedSnowflakeStreamingAdmissionDestinations(t), cfg)
	if err == nil {
...
func TestManagedAdmissionStreamingAppendRejectsMisconfigBeforeTransportRefusal(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "transactional write mode", key: "write_mode", value: "target", want: "write_mode=streaming_append"},
