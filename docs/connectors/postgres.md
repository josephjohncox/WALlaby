...
For least privilege, have an administrator create the slot and publication, grant the runtime role `REPLICATION` and table reads, then use native booleans in the typed source branch:
...
| --- | --- | --- | --- |
| `connection.dsn` | yes | — | PostgreSQL connection string. Treat it as a secret. |
| `slot` | yes | — | Logical replication slot name. |
| `publication` | yes | — | Publication name. |
| `publication_tables` | recommended | all tables when a new publication has no explicit scope | Native list of `schema.table` strings. Set this explicitly. |
...
| `mode` | yes | — | `POSTGRES_SOURCE_MODE_CDC` or `POSTGRES_SOURCE_MODE_BACKFILL`. CDC publication selection and backfill selection are mutually exclusive. |
...
| `managed_profile` | no | — | Exact protobuf enum; the maintained PostgreSQL profile uses `MANAGED_PROFILE_POSTGRES_TO_POSTGRES_V1` only with its full contract. |
...
In CDC mode, `publication_tables` and `publication_schemas` never fall back to `backfill_tables` or `backfill_schemas`. If both publication selectors are absent, an automatically created publication covers all tables. That default is convenient for a disposable database and dangerous for a shared database. Production flow files should name their table scope.
...
| Option | Required | Default | Meaning |
| --- | --- | --- | --- |
| `connection.dsn` | yes | — | Target PostgreSQL connection string. Treat it as a secret. |
| `synchronous_commit` | no | server setting | Transaction durability setting for destination writes. |
| `metadata.enabled` | no | `true` | Maintain WALlaby's destination metadata table. |
| `managed_profile` | no | — | Exact protobuf enum; it must match the source profile for maintained managed execution. |
...
| Option | Required | Default | Meaning |
| --- | --- | --- | --- |
| `connection.dsn` | yes | — | PostgreSQL database used for stream storage. |
