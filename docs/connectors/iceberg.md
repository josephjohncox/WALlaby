...
Stable identity is persisted in each Iceberg field's doc as immutable commit metadata, and the deterministic mapping fingerprint is recorded in the snapshot summary, so a retry rebuilds the same mapping from the catalog rather than inferring renames by name.

Wallaby creates missing tables as unpartitioned Iceberg v2 tables and evolves existing tables additively; a new canonical column is added as a nullable Iceberg column and a supported rename is applied by stable identity. Existing partitioned tables are accepted only when every partition source field still resolves in the current table schema. Incompatible type changes and ambiguous renames fail closed. The schema evolution and the data append are committed in the same catalog transaction so schema and data recovery are not split across two effects.
...
The endpoint may override non-secret catalog and target mapping values. Supply OAuth tokens, OAuth credentials, client certificates, CA data, and AWS credentials through deployment configuration or environment variables.
...
- static bearer tokens and OAuth client credentials;
...
Data and metadata objects are read and written through the catalog-reported object store. For S3-compatible storage such as MinIO in local emulation, set `s3_endpoint` and `s3_region` flow options; access and secret keys are secrets and come only from deployment configuration or the AWS environment. These client-side settings let the committer reach the same object store the catalog uses even when the catalog reports a different in-cluster endpoint.

## S3 Tables profile

Set `catalog_profile=s3tables`. The client signs the AWS Glue Iceberg REST endpoint with signing name `glue`. The warehouse uses the AWS form `<account-id>:s3tablescatalog/<table-bucket-name>`.
